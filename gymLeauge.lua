...
local rmtev = game:GetService("ReplicatedStorage"):WaitForChild("common"):WaitForChild("packages"):WaitForChild("_Index"):WaitForChild("sleitnick_knit@1.5.1"):WaitForChild("knit"):WaitForChild("Services")

-- Variables
local isLoopActive = false -- Tracks whether the loop is currently running
local connection = nil -- Stores the loop's connection

-- Function to start the loop
local function startLoop()
    if connection then return end -- Avoid starting multiple loops
    isLoopActive = true
    print("Loop started!")

    connection = RunService.Heartbeat:Connect(function()
	rmtev:WaitForChild("EquipmentService"):WaitForChild("RF"):WaitForChild("AutoLoad"):InvokeServer()
	task.wait(0.3)
	rmtev:WaitForChild("EquipmentService"):WaitForChild("RE"):WaitForChild("click"):FireServer()
	task.wait(0.3)
	rmtev:WaitForChild("CombatService"):WaitForChild("RF"):WaitForChild("M1"):InvokeServer()
    end)
end

-- Function to stop the loop
local function stopLoop()
    if not connection then return end -- Avoid stopping a non-existent loop
    isLoopActive = false
...
--gear spin
local function gearspn()
	local args = {
	"Basic Crate",
	false
	}
	rmtev:WaitForChild("RouletteService"):WaitForChild("RF"):WaitForChild("Roll"):InvokeServer(unpack(args))

end

--bolt spin
local function boltspn()
	local args = {
	"forCurrency"
	}
	rmtev:WaitForChild("LimitedTimeEventService"):WaitForChild("RE"):WaitForChild("buySpin"):FireServer(unpack(args))
...
--tab3 btn
zbtn8.MouseButton1Click:Connect(gearspn)
zbtn9.MouseButton1Click:Connect(boltspn)
zbtn10.MouseButton1Click:Connect(function(input, gameProcessedEvent)
    -- Don't run if the user is typing in a chat box
