...
--// MODIFIED TARGETING FUNCTION TO ZOMBIES
...
local GetClosestZombie = function()
	...
	if ZombiesFolder then
		for _, Zombie in ipairs(ZombiesFolder:GetChildren()) do
			local Character = Zombie
			local Humanoid = Character and FindFirstChildOfClass(Character, "Humanoid")
			local LockPartInstance = Character and FindFirstChild(Character, LockPart)

			if Humanoid and LockPartInstance and Humanoid.Health > 0 then
				local PartPosition = LockPartInstance.Position

				-- WallCheck (optional, can be omitted if not needed)
				...
				if Distance < RequiredDistance and OnScreen then
					RequiredDistance, Environment.Locked = Distance, Zombie
				end
			end
		end
	end
...
local Load = function()
	...
	ServiceConnections.RenderSteppedConnection = Connect(__index(RunService, Environment.DeveloperSettings.UpdateMode), function()
		...
		if Running and Settings.Enabled then
			...
			Offset = OffsetToMoveDirection and __index(FindFirstChildOfClass(__index(Environment.Locked, "Humanoid"), "Humanoid"), "MoveDirection") * (mathclamp(Settings.OffsetIncrement, 1, 30) / 10) or Vector3zero
...
function Environment.Exit(self)
	...
	Load = nil; ConvertVector = nil; CancelLock = nil; GetClosestZombie = nil; GetRainbowColor = nil; FixUsername = nil
...
function Environment.GetClosestZombie()
	GetClosestZombie()
